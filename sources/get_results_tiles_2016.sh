...
precincts_voting;votes;votes_pct;votes_office" | \
  cat - <(wget -O - -o /dev/null https://electionresults.sos.state.mn.us/Results/MediaResult/100?mediafileid=52) > mn-pres-precinct-2016.tmp.csv &&

echo "Getting DFL totals ..." &&
cat mn-pres-precinct-2016.tmp.csv | \
  csv2json -r ";" | \
  ndjson-split | \
...
cat mn-pres-precinct-2016.tmp.csv | \
  ...
  ndjson-join 'd.id' 'd.county_id + d.precinct_id' - <(cat mn-pres-precinct-2016.tmp.csv | csv2json -r ";" | ndjson-split) | \
...
ndjson-map '{"id": d.id, "winner": d.winner, "winner_margin": d.winner_margin, "dfl_votes": d.dfl_votes, "gop_votes": d.gop_votes, "oth_votes": d.oth_votes, "total_votes": d.total_votes, "dfl_pct": d.dfl_pct, "gop_pct": d.gop_pct, "oth_pct": d.oth_pct, "winner_cat": d.winner != "even" && d.winner != "oth" ? d.winner_margin <= 5 ? d.winner + "-small" : d.winner_margin <= 25 && d.winner_margin > 5 ? d.winner + "-med" : d.winner_margin > 25 ? d.winner + "-large" : null : d.winner == "oth" ? "oth" : "even"}' > joined.tmp.ndjson &&
